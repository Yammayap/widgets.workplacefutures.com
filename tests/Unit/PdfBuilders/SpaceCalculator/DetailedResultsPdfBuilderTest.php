...
class DetailedResultsPdfBuilderTest extends TestCase
{
    public function test_pdf_is_returned()
    {
        ...
        $builder = app()->make(DetailedResultsPdfBuilder::class);

        $pdf = $builder->build($enquiry);

        $this->assertEquals('pdfs.space-calculator.detailed', $pdf->viewName);
    }
}
